...
public class BatchConfiguration {
	...
	public Flow flow1() {
		...
		flow.start(sbf.get("step1").tasklet(tasklet()).build());
	...
	public Flow flow2() {
		...
		flow.start(sbf.get("step2").tasklet(tasklet()).build())
		.next(sbf.get("step3").tasklet(tasklet()).build());
	...
	public static class CountingTasklet implements Tasklet {
		...
		public RepeatStatus execute(StepContribution contribution, ChunkContext chunkContext) throws Exception {
			System.out.println(String.format("%s has been executed on thread %s", 
					chunkContext.getStepContext(), Thread.currentThread().getName()));
