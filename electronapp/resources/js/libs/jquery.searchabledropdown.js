...
(function(jq) {
	...
	plugin.execute = function(settings, zindex) {
		...
		if (navigator.appName.msie && parseInt(navigator.appName.version) < 7)
			return this;
        ...
        // detecting chrome
        navigator.appName.chrome = /chrome/.test(navigator.userAgent.toLowerCase());
        if(navigator.appName.chrome) navigator.appName.safari = false;
        ...
        if (jq.meta){
            settings = jq.extend({}, options, self.data());
        }
        ...
        var overlay = jq("<div/>");
        var input = jq("<input/>");
        var selector = jq("<select/>");

        // matching option items
        var topMatchItem = jq("<option>"+settings.warnMultiMatch.replace(/\{0\}/g, settings.maxMultiMatch)+"</option>").attr("disabled", "true");
        var noMatchItem = jq("<option>"+settings.warnNoMatch+"</option>").attr("disabled", "true");
        ...
        var selectorHelper = {
             ...
             */
			option: function(idx) {
	        	return jq(selector.get(0).options[idx]);
	        },
	        /**
	        ...
	        reset: function() {
	            ...
	            for (var i=begin; i < end; i++)
	            	selector.append(jq(self.get(0).options[i]).clone().attr(idxAttr, i-1));
        ...
        selector.mousemove(function(e) {
        	...
        	if(navigator.appName.opera)
        		fsdiff = 2.5;
        	if(navigator.appName.safari || navigator.appName.chrome)
        		fsdiff = 3;
        ...
        input.keyup(function(e) {
        	...
        	// set search text
        	search = jq.trim(input.val().toLowerCase());
        ...
        function draw() {
    		...
    		// IE7
    		if(navigator.appName.msie && parseInt(navigator.appName.version) < 8) {
...
*/
        ...
        function searching() {
            ...
            for(var i=1;i<self.get(0).length && matches < settings.maxMultiMatch;i++){
                ...
                if(search.length == 0 || search.test(self.get(0).options[i].text)){
                	var opt = jq(self.get(0).options[i]).clone().attr(idxAttr, i-1);
    ...
    function register(nsp) {
    	...
    	jq.fn[nsp] = function(settings) {
    		// extend default settings
    		settings = jq.extend(plugin.defaults, settings);
